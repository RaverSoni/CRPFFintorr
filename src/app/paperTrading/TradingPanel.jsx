...
const TradingPanel = () => {
  ...
  // Fetch stock and buying power data automatically every 30 seconds
  useEffect(() => {
    const fetchStockInfo = async () => {
      try {
        const response = await fetch(`http://127.0.0.1:8000/api/stock_info/${ticker}/`);
  ...
  if (!stockData) return <div className="text-gray-500 text-center">Loading...</div>;
  ...
  const estimatedTotal = (quantity * marketPrice).toFixed(2);
  const estimatedLoss = useStopPrice ? ((marketPrice - stopPrice) * quantity).toFixed(2) : "0.00";
  const currencySymbol = stockData.currencySymbol || "$";
  ...
  return (
    <div className="bg-black p-6 rounded-lg shadow-md text-white w-full">
      ...
      <form onSubmit={executeTrade} className="space-y-4">
        ...
        {/* Estimated Loss */}
        <div className={`text-lg font-semibold ${tradeType === "buy" ? "text-red-400" : "text-green-400"}`}>
          Est. Loss: {currencySymbol}{estimatedLoss}
        </div>
        ...
        <div className="bg-gray-800 p-4 rounded-lg text-sm">
          <p className="flex justify-between">
            <span className="text-gray-400">Buying Power</span>
            <span className="font-semibold">{currencySymbol}{buyingPower.toLocaleString()}</span>
          </p>
          <p className="flex justify-between">
            <span className="text-gray-400">Transaction Fees</span>
            <span className="font-semibold">{currencySymbol}{transactionFee}</span>
          </p>
          <p className="flex justify-between">
            <span className="text-gray-400">Estimated Total</span>
            <span className="font-semibold">{currencySymbol}{estimatedTotal}</span>
          </p>
        </div>
